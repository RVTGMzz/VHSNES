...
# Real-Vietnamese typography probe. Every replacement preserves the exact
# two-byte character count of the original Japanese field.
MENU_FIELDS = (
    (0x28818, "ストーリーモード", "Âm thanh"),  # 8 units
    (0x2882E, "対戦モード", "Bói!!"),          # 5 units
    (0x2883C, "チーム対戦モード", "Đấu đội!"), # 8 units
    (0x28852, "まるこＱ", "Vẽ!!"),             # 4 units
    (0x2885E, "まるこペイント", "Maruko?"),    # 7 units
    (0x28870, "まるこみくじ", "Ổn rồi"),       # 6 units
)
...
def unpack_bitmap_hex(bitmap_hex: str) -> list[str]:
    ...
    return [
        "".join("#" if bits[y * 12 + x] else "." for x in range(12))
        for y in range(12)
    ]
...
def load_codepage(path: Path) -> tuple[dict[str, bytes], dict[str, int], dict[str, str], dict[str, str]]:
    enc: dict[str, bytes] = {}
    gid: dict[str, int] = {}
    visual: dict[str, str] = {}
    source: dict[str, str] = {}
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        for row in csv.DictReader(f):
            ...
            source[ch] = row["glyph_source"]
    return enc, gid, visual, source
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Probe 009: FE4 Vietnamese-reference font transfer on Chibi menu")
    ap.add_argument("rom", type=Path)
    ap.add_argument("output", type=Path)
    here = Path(__file__).resolve().parent
    ap.add_argument("--codepage", type=Path, default=here.parent / "translation" / "codepage" / "vi_codepage_v3_fe4ref.csv")
    ap.add_argument("--glyphs", type=Path, default=here.parent / "translation" / "codepage" / "vi_glyphs_v3_fe4ref.json")
    ...
    for ch, key in visual_key.items():
        ...
        if key not in glyph_json:
            raise RuntimeError(f"custom visual key missing from glyph JSON: {key!r}")
        g = char_gid[ch]
    ...
    replacements: list[tuple[int, bytes, bytes, str]] = []
    for off, source_jp, vi in MENU_FIELDS:
        ...
        replacements.append((off, source, repl, vi))
    ...
    for off, _src, _repl, vi in replacements:
        print(f"probe_row 0x{off:06X}: {vi}")
    print("expected_runtime_rows=Âm thanh / Bói!! / Đấu đội! / Vẽ!! / Maruko? / Ổn rồi  # FE4-ref glyph style")
    ...
    for off, source, repl, _vi in replacements:
